...
(function () {
  ...
  function setupCustomSelect(id) {
    ...
    // init UI
    const labelEl = toggle.querySelector('.label-text');
    ...
    // ensure options focusable and reattach listeners (clone to remove duplicates)
    Array.from(list.querySelectorAll('li')).forEach(li => {
      ...
      node.addEventListener('click', (ev) => {
        ...
        hidden.value = v;
        const lbl = newToggle.querySelector('.label-text');
        if (lbl) lbl.textContent = node.textContent.trim();
    ...
    // stopPropagation on list while interacting
  ...
  function attachGlobalClose() {
    ...
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') selects.forEach(s => s.wrap.classList.remove('open'));
    });
  }
  ...
  function attachRipple() {
    document.addEventListener('click', function (e) {
      ...
      circle.style.position = 'absolute'; circle.style.borderRadius = '50%'; circle.style.background = 'rgba(255,255,255,0.10)';
      circle.style.pointerEvents = 'none'; circle.style.transform = 'scale(0)'; circle.style.transition = 'transform .45s ease, opacity .45s ease';
